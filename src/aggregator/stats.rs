...
pub struct SessionSummary {
    ...
    pub slug: String,
    /// Title generated by Claude Code (`ai-title` entry), if any.
    pub title: String,
...
impl SessionSummary {
    /// Best available human-readable label: AI title, then slug, then the
    /// first prompt, then a shortened id.
    pub fn display_title(&self) -> String {
        [&self.title, &self.slug, &self.first_prompt]
            .into_iter()
            .find(|s| !s.is_empty())
            .cloned()
    ...
    /// First 8 characters of the session id, with an ellipsis if truncated.
    pub fn short_id(&self) -> String {
        match self.session_id.char_indices().nth(8) {
            Some((idx, _)) => format!("{}…", &self.session_id[..idx]),
...
pub fn aggregate_project(
    ...
    let projects = discover_projects(claude_dir)?;
    match projects.into_iter().find(|p| p.dir_name == dir_name) {
        Some(project) => aggregate_project_dir(&project, time_range).map(Some),
        None => Ok(None),
    }
}

/// Aggregate detailed stats for an already-discovered project.
pub fn aggregate_project_dir(
    project: &ProjectDir,
    time_range: TimeRange,
) -> Result<ProjectDetailStats> {
    let cutoff = time_range.cutoff();
            ...
            if let Some(common) = entry.common() {
                if let Some(sid) = &common.session_id {
                    ...
                    if let Some(ts) = common.timestamp {
                        ...
                        if sess.last_active.is_none_or(|prev| ts > prev) {
                            sess.last_active = Some(ts);
                        }
                    }
                }
            }

            // Session title (the latest ai-title entry wins)
            if let TranscriptEntry::AiTitle(t) = entry
                && let (Some(sid), Some(title)) = (&t.common.session_id, &t.ai_title)
                && let Some(sess) = session_map.get_mut(sid)
            {
...
mod tests {
    ...
    #[test]
    fn test_session_display_title_fallback_chain() {
        let mut sess = SessionSummary {
            session_id: "2485c638-eede-49ff-85fa-b650c60a3120".to_string(),
            ..Default::default()
        };
        assert_eq!(sess.display_title(), "2485c638…");

        sess.first_prompt = "fix the failing test".to_string();
        assert_eq!(sess.display_title(), "fix the failing test");

        sess.slug = "sunny-otter".to_string();
        assert_eq!(sess.display_title(), "sunny-otter");

        sess.title = "Add sessions command".to_string();
        assert_eq!(sess.display_title(), "Add sessions command");
    }
}
